...
#include <asm/apic.h>
#endif
void jiajia_printf(const char* format, ...);
...
static int jailhouse_cmd_enable(struct jailhouse_system __user *arg)
{
	...
	if (err) {
		//jiajia_printf("jiajia: Failed to load hypervisor image %s, err = %d\n", fw_name, err);
		pr_err("jailhouse: Missing hypervisor image %s\n", fw_name);
		goto error_put_module;
	}

	header = (struct jailhouse_header *)hypervisor->data;
	jiajia_printf("jiajia1: Loaded hypervisor image, core size = %u, percpu size = %u\n",
              header->core_size, header->percpu_size);

	err = -EINVAL;
	if (memcmp(header->signature, JAILHOUSE_SIGNATURE,
		   sizeof(header->signature)) != 0 ||
	    hypervisor->size >= hv_mem->size){
		//jiajia_printf("jiajia: Hypervisor signature mismatch or hypervisor size too large\n");
		goto error_release_fw;
		}

	hv_core_and_percpu_size = header->core_size +
		max_cpus * header->percpu_size;
	jiajia_printf("jiajia2: hv_core_and_percpu_size = %lu\n", hv_core_and_percpu_size);
	...
	jiajia_printf("jiajia3: Hypervisor memory successfully mapped, console_page = %p\n", console_page);
	...
	jiajia_printf("jiajia: I-cache flushed from %lx to %lx\n",
              (unsigned long)hypervisor_mem, (unsigned long)(hypervisor_mem + header->core_size));
	...
	if (config->debug_console.clock_reg) {
		...
		if (!clock_reg) {
			...
			jiajia_printf("jiajia: Failed to map clock register at %08lx\n",
		              (unsigned long)config->debug_console.clock_reg);

			goto error_unmap;
		}
		jiajia_printf("jiajia: Clock register mapped at %p\n", clock_reg);
	...
	if (CON_IS_MMIO(config->debug_console.flags)) {
		...
		if (!console) {
			...
			jiajia_printf("jiajia: Failed to map debug console at %08lx\n",
			              (unsigned long)config->debug_console.address);
			goto error_unmap;
		}
		/* The hypervisor has no notion of address spaces, so we need
		 * to enforce conversion. */
		jiajia_printf("jiajia: Debug console mapped at %p\n", console);
...
#endif
#ifdef CONFIG_MACH_AGX_ORIN
jiajia_printf("CONFIG_MACH_AGX_ORIN is defined\n");
#endif
	...
	jiajia_printf("jiajia: Online CPUs = %u\n", header->online_cpus);
	...
	while (atomic_read(&call_done) != num_online_cpus())
		cpu_relax();
	jiajia_printf("jiajia: All CPUs have entered hypervisor\n");
	...
	jiajia_printf("jiajia: Jailhouse is successfully enabled.\n");
